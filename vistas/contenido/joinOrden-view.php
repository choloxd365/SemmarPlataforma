...
$this->setTextColor(0);
    ...
    function Body() {
        ...
        $this->MultiCell(160,14, 'Chimbote '.$fechaActual,0, 'R');
...
//*************************************************************************//
        ...
        $this->SetFont('arial', 'B', 8);
        $this->Cell(137,5,'ATENCION', 0,1,'L');
        ...
        $this->SetXY(10, $y);
        $this->MultiCell(15, 5, utf8_decode("ITEM"), 1, 'C',1); //Utilizamos el utf8_decode para evitar código basura o ilegible
        $this->SetXY(25, $y); //El resultado 22 es la suma de la posición 10 y el tamaño del MultiCell de 12.
        $this->MultiCell(85, 5, utf8_decode("DESCRIPCION"), 1, 'C',1);
        $this->SetXY(110, $y);
        $this->MultiCell(20, 5, utf8_decode("UNIDAD"), 1, 'C',1);
        $this->SetXY(130, $y);
        $this->MultiCell(18, 5, utf8_decode("CANTIDAD"), 1, 'C',1);
        $this->SetXY(148, $y);
        $this->MultiCell(25, 5, utf8_decode("PRECIO/UNIDAD"), 1, 'C',1);  
        $this->SetXY(173, $y);
        $this->MultiCell(27, 5, utf8_decode("PRECIO/TOTAL"), 1, 'C',1);      
        ...
        require_once "./../../core/mainModel.php";
        $result = mainModel::ejecutar_consulta_simple("SELECT ord.moneda_ord,subtotal_ord,igv_ord,total_ord,de.desc_ord,de.unidad_ord,de.cantidad_ord,de.precio_uni,de.precio_total FROM `ordencompra` ord INNER JOIN detalleorden de ON ord.id_orden=de.id_orden WHERE ord.id_orden='$id_orden' ORDER BY de.desc_ord ASC");
        $numero=0;
        $subtotal=0;
        $igv=0;
        $total=0;
     ...
     //Redondeamos el resultado y lo multiplicamos por el alto de las filas
        ...
        $this->SetXY(148, $y);
        $this->MultiCell(25, $m, $moneda.utf8_decode(mainModel::moneyFormat($row['precio_uni'],"USD") ), 1, 'C',1);  
        $this->SetXY(173, $y);
        $this->MultiCell(27, $m, $moneda.utf8_decode(mainModel::moneyFormat( $row['precio_total'],"USD")), 1, 'C',1);      
        ...
        //IGV
        if ($igv!=0) {
          $this->MultiCell(27, 6, $moneda.utf8_decode(mainModel::moneyFormat($subtotal,"USD")), 1, 'C',1);   
          $y = $this->GetY();
          $this->SetXY(148, $y);
        ...
        $this->setTextColor(255,255,255);
        $this->SetDrawColor(255,255,255);
        $this->MultiCell(25, 6, utf8_decode("IGV 18%"), 1, 'C',1);  
        $this->setTextColor(0);
        $this->SetDrawColor(217,220,255);
        $this->SetFillColor(217,220,255);
        ...
        $this->SetXY(173, $y);
        $this->MultiCell(27, 6, $moneda.utf8_decode(mainModel::moneyFormat($igv,"USD")), 1, 'C',1);   
        $y = $this->GetY();
        $this->SetXY(148, $y);
        ...
        $this->SetXY(173, $y);
        $this->MultiCell(27, 6, $moneda.utf8_decode(mainModel::moneyFormat($total,"USD")), 1, 'C',1);  
        }else{
          
        $this->MultiCell(27, 6, $moneda.utf8_decode(mainModel::moneyFormat($subtotal-($subtotal*0.18),"USD")), 1, 'C',1);   
        $y = $this->GetY();
        $this->SetXY(148, $y);
        $this->SetFillColor(61,70,255);
        $this->setTextColor(255,255,255);
        ...
        $this->SetXY(173, $y);
        $this->MultiCell(27, 6, $moneda.utf8_decode(mainModel::moneyFormat($subtotal*0.18,"USD")), 1, 'C',1);   
        $y = $this->GetY();
        $this->SetXY(148, $y);
        ...
        if (isset($_GET['id_user'])){
            $id_sesion=$_GET['id_user'];
            }

            
        require_once "./../../core/mainModel.php";
        $result = mainModel::ejecutar_consulta_simple("SELECT nombres FROM usuario WHERE id_usuario='$id_sesion'");
        $numero=0;
        ...
        $this->SetXY(50, $y);
         $this->Cell(100, 50,utf8_decode("Atentamente,  "), 0, 1, 'C');
         
        $this->SetXY(50, $y+5);
         $this->Cell(100, 50,utf8_decode($nombres), 0, 1, 'C');
        
        $this->Image('homologaciones/hodelpe.png',180,280,23);
        $this->Image('homologaciones/sgs.png',135,277,43);
...
$phpMailer = new PHPMailer();
 ...
 $phpMailer->AddStringAttachment($pdf->Output('','S'), 'orden/'.$id_orden.'.pdf', 'base64', 'application/pdf');
